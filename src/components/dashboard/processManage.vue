<template>
  <div class="processManage">
            ...
            <el-dropdown trigger="click" class='fr'>
              ...
              <el-dropdown-menu slot="dropdown">
                <el-checkbox-group v-model="checkList" @change="changeArr">
                  <div class='lab_div'>
                    <el-checkbox label="公司编号"></el-checkbox>
                  </div>
                  <div class='lab_div'>
                    <el-checkbox label="工序名称"></el-checkbox>
                  </div>
                  <div class='lab_div'>
                    <el-checkbox label="所需技能"></el-checkbox>
                  </div>
              </el-checkbox-group>
              </el-dropdown-menu>
            </el-dropdown>
    ...
    <el-table style="width: 100%" :data="pager.records">
      ...
      <!-- <template v-for='(val,key,index) in objArr'>
        <el-table-column :prop="key" :label="val" width="160" :key='index'></el-table-column>
      </template> -->
      <el-table-column prop="companyId" label="公司编号" width="160"></el-table-column>
...
export default {
  data() {
    return {
      ...
      objArr:[{companyId:"公司编号"},{name:'工序名称'},{skill:'所需技能'}],
      page: 1,
      ym_val: null,
      users: [],
  ...
  methods: {
    ...
    changeArr(val){
      ...
      for(let i = 0;i<this.objArr.length;i++){
        ...
        for( j in obj){
          let isBool = val.indexOf(obj[j]);
          if(isBool !== -1){
            linshiArr.push(obj)
          }
          // console.log(isBooleans);
        }
      }
      this.checkList = linshiArr;
      console.log(this.checkList)
    },
    inquire(){
      var that = this;
      this.axios.post('/api/procedure/findList',qs.stringify(that.fliters.name)).then(res => {
          this.$store.state.processManageData = res.data.result;
          this.pager = this.$store.state.processManageData;
      }).catch(err => {
        console.log(err)
